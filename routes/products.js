...
router.post('/', authAdmin, async (req, res) => {
  const { slug, name, category, tag, emoji, short_desc, full_desc, spice, benefits, ingredients, prices, images, rating, reviews, coupon_applicable, subcategory } = req.body;
  try {
...
router.put('/:id', authAdmin, async (req, res) => {
  const { name, category, tag, emoji, short_desc, full_desc, spice, benefits, ingredients, prices, images, in_stock, rating, reviews, coupon_applicable, subcategory } = req.body;
  try {
    const reviewsCount = Array.isArray(reviews) ? reviews.length : (parseInt(reviews) || 0);
    const ratingVal = parseFloat(rating) || 0;
    const result = await pool.query(
